...
export async function sendFile(req, res, ext) {
    const { id = '' } = req.params;
    // Request validation
    try {
    ...
    let blob_size_parsed = undefined;
    let attempts = 0;
            ...
            try {
                ...
                if (blob_size_parsed) {
                    if (blob_size_parsed > BLOB_SIZE_LIMIT) {
                        ...
                        res.status(404).send(`Blob size ${blob_size_parsed} not supported by Suiftly (${BLOB_SIZE_LIMIT} limit)`);
                        return;
                    }
                }
            }
    ...
    let requestedFilePath = undefined;
    if (auxFilePath) {
