...
const ROUTES = ['blob', 'icon48x48', 'icon96x96', 'icon256x256', 'meta', 'metrics', 'view'];
...
function convertDateToURLFragment(date) {
...
async function downloadMissingFiles(missingDates) {
...
async function deleteOldFiles(dir, expiration) {
...
async function gunzipFile(gzPathname, outputDir) {
    ...
    try {
        ...
        for (const file of files) {
            ...
            for await (const line of rl) {
                ...
                if (parts.length < 12)
                    continue; // Ensure there are enough parts
                const statusCode = parts[1] || '';
                if (statusCode !== '200')
                    continue; // Skip lines with non-200 status code
                const url = parts[7] || '';
                ...
                if (!route || !blobId)
                    continue; // Skip lines with missing URL fragments
                ...
                // in the .metrics file.
                const routeMetrics = dayMetrics[routeType] ||
            ...
            for (const blobId in metricsMap) {
                ...
                for (const route in blobMetrics.sizes) {
                    if (blobMetrics.sizes[route] === 0) {
                        delete blobMetrics.sizes[route];
                    }
                }
                    ...
                    if (Object.prototype.hasOwnProperty.call(blobMetrics.daily, day)) {
                        ...
                        for (const route in dayMetrics) {
                            if (Object.prototype.hasOwnProperty.call(dayMetrics, route)) {
                                const routeMetrics = dayMetrics[route];
                                if (routeMetrics) {
